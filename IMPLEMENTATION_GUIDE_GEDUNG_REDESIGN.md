# 🛠️ IMPLEMENTATION GUIDE - REDESIGN GEDUNG KARYAWAN

## 📋 OVERVIEW

File target: `resources/views/fasilitas/gedung/index-karyawan.blade.php`

Transform dari **Horizontal Pricing Card** → **Grid Admin Dashboard Style**

---

## 🔧 STEP-BY-STEP IMPLEMENTATION

### **STEP 1: Update Controller (Load Room Data)**

**File:** `app/Http/Controllers/GedungController.php`
...
public function indexKaryawan()
{
    ...
    $gedung = Gedung::with([
        'cabang',
        'ruangans' => function($query) {
            $query->select('id', 'gedung_id', 'nama_ruangan', 'kode_ruangan', 'foto', 'kapasitas')
                  ->withCount('barangs as total_barang')
    ...
    ->withCount('ruangans as total_ruangan')
    ->withCount('barangs as total_barang')
    ->paginate(12); // Increase to 12 for grid layout

    return view('fasilitas.gedung.index-karyawan', compact('gedung'));
}
```

---

### **STEP 2: HTML Structure (Blade Template)**

**File:** `resources/views/fasilitas/gedung/index-karyawan.blade.php`

#### **A. Header Section (Keep Same)**
```blade
...
#### **B. Content Section (MAJOR CHANGE)**
                ...
                <div class="gedung-card-wrapper">
                    ...
                    <div class="gedung-card {{ $variantClass }}" data-gedung-id="{{ $d->id }}">
                        ...
                        <div class="card-footer">
                            <button class="btn-view-rooms {{ $variantClass }}" 
                    ...
                    {{-- Room List Panel --}}
                    <div class="room-list-panel" id="room-list-{{ $d->id }}">
                                    ...
                                    <div class="room-item">
                                        ...
                                        <div class="room-actions">
                                            <button class="btn-action btn-view {{ $variantClass }}" 
                                                    onclick="viewRoomDetail({{ $room->id }}, '{{ $room->nama_ruangan }}')">
                                                <ion-icon name="eye-outline"></ion-icon>
                                            </button>
                                            <a href="{{ route('barang.karyawan', ['gedung_id' => Crypt::encrypt($d->id), 'ruangan_id' => Crypt::encrypt($room->id)]) }}">
                                                <button class="btn-action btn-navigate {{ $variantClass }}">
...
<script>
    ...
    function viewRoomDetail(roomId, roomName) {
        // Show modal dengan SweetAlert2
        Swal.fire({
            title: roomName,
            html: `
                <div style="text-align: left;">
                    <p><strong>Kode Ruangan:</strong> R-${roomId}</p>
                    <p><strong>Status:</strong> Aktif</p>
                    <p><strong>Kapasitas:</strong> 20 orang</p>
                    <p><strong>Total Barang:</strong> 45 item</p>
                    <p style="margin-top: 15px;">
                        <em>Detail lengkap dapat diakses dengan mengklik tombol navigasi.</em>
                    </p>
                </div>
            `,
            icon: 'info',
...
- [ ] "Lihat Ruangan" button expands room list
- [ ] Room list shows all rooms
- [ ] Eye button on room shows detail modal
- [ ] Arrow button navigates to room page
- [ ] Close button collapses room list
...
### **4. Dark Mode Testing**
- [ ] Toggle dark mode
- [ ] Check all colors readable
- [ ] Check shadows visible
- [ ] Check gradients display correctly
...
```bash
cp resources/views/fasilitas/gedung/index-karyawan.blade.php resources/views/fasilitas/gedung/index-karyawan-OLD.blade.php
```

2. **Update Controller**
```bash
# Edit: app/Http/Controllers/GedungController.php
# Add eager loading for rooms
```

3. **Replace Blade Template**
```bash
# Replace content of index-karyawan.blade.php
# With new HTML structure + CSS + JS
```
...
```bash
git add .
git commit -m "Redesign: Gedung karyawan page to grid layout"
git push
```
...
⚠️ **Important:**
- Make sure `Ruangan` model has relationship with `Barang` for count
- Verify `Crypt::encrypt()` is working for route parameters
- Test with real data (gedung with and without rooms)
- Consider adding loading states for AJAX room loading

💡 **Enhancements:**
- Add search bar in header
- Add filter by cabang
- Add sort options
- Lazy load room data (AJAX on panel open)
- Add room photo preview in list
