<div class="row">
    <div class="col">
        <table class="table">
            <tr>
                <th>Kode Yayasan</th>
                <td class="text-end">{{ $yayasan->kode_yayasan }}</td>
            </tr>
            <tr>
                <th>Nama Yayasan</th>
                <td class="text-end">{{ $yayasan->nama }}</td>
            </tr>
            <tr>
                <th>Dept</th>
                <td class="text-end">{{ $yayasan->kode_dept }}</td>
            </tr>
            <tr>
                <th>Cabang</th>
                <td class="text-end">{{ $yayasan->kode_cabang }}</td>
            </tr>
        </table>
    </div>
</div>
<div class="row mt-2">
    <div class="col">
        <form action="{{ route('yayasan-presensi.update') }}" method="POST" id="formEditPresensi">
            @csrf

            <input type="hidden" value="{{ Crypt::encrypt($yayasan->kode_yayasan) }}" name="kode_yayasan">
...
<script>
    ...
    $("#formEditPresensi").submit(function(e) {
        ...
        // Format jam_in & jam_out jika belum include tanggal
        if (jam_in && jam_in.indexOf(' ') === -1) {
            jam_in = tanggal + ' ' + jam_in;
            $(this).find("#jam_in").val(jam_in);
        }
        if (jam_out && jam_out.indexOf(' ') === -1) {
            jam_out = tanggal + ' ' + jam_out;
            $(this).find("#jam_out").val(jam_out);
        }
        ...
        if (status == "") {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'Oops...',
                text: 'Jam Absen Masuk dan Pulang Harus Diisi!',
                didClose: () => {
