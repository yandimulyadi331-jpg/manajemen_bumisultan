@extends('layouts.app')
@section('titlepage', 'Edit Transaksi Keuangan')

@section('content')
@section('navigasi')
    <span><a href="{{ route('keuangan-santri.index') }}">Dompet Santri</a> / Edit Transaksi</span>
@endsection
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('keuangan-santri.update', $transaction->id) }}" enctype="multipart/form-data" id="formTransaksi">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group mb-3">
                                ...
                                <input type="date" name="tanggal_transaksi" value="{{ old('tanggal_transaksi', $transaction->tanggal_transaksi->format('Y-m-d')) }}" 
                    ...
                    <div class="row">
                                ...
                                <div class="mt-2">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="jenis" id="pemasukan" 
                                               value="pemasukan" {{ old('jenis', $transaction->jenis) == 'pemasukan' ? 'checked' : '' }}>
                                    ...
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="jenis" id="pengeluaran" 
                                               value="pengeluaran" {{ old('jenis', $transaction->jenis) == 'pengeluaran' ? 'checked' : '' }}>
                        ...
                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" name="jumlah" value="{{ old('jumlah', $transaction->jumlah) }}" 
                        ...
                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group mb-3">
                                ...
                                <select name="metode_pembayaran" class="form-select">
                                    ...
                                    <option value="E-Wallet" {{ old('metode_pembayaran', $transaction->metode_pembayaran) == 'E-Wallet' ? 'selected' : '' }}>E-Wallet (OVO, GoPay, Dana)</option>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea name="catatan" rows="3" class="form-control" placeholder="Catatan tambahan...">{{ old('catatan', $transaction->catatan) }}</textarea>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        @if($transaction->bukti_file)
                            <div class="mb-2">
                                ...
                                <a href="{{ Storage::url($transaction->bukti_file) }}" target="_blank" class="text-primary">
                                    <i class="ti ti-file me-1"></i>Lihat File
                                </a>
                            </div>
                        @endif
                        <input type="file" name="bukti_file" accept="image/*,application/pdf" class="form-control">
                        <small class="text-muted">Maksimal 2MB. Format: JPG, PNG, PDF. Kosongkan jika tidak ingin mengubah.</small>
                    </div>

                    <!-- Buttons -->
                    <div class="form-group mt-4">
                        <button type="button" id="btnUpdateTransaksi" class="btn btn-primary">
...
document.getElementById('btnUpdateTransaksi').addEventListener('click', function() {
    Swal.fire({
...
// SweetAlert untuk konfirmasi Kembali
document.getElementById('btnKembali').addEventListener('click', function() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "{{ route('keuangan-santri.show', $transaction->santri_id) }}";
        }
    });
});
