...
<div class="page-body">
    <div class="container-xl">
        <div class="row">
            <div class="col-12">
                <form action="{{ route('pelanggaran-santri.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Form Pelanggaran Santri</h3>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="user_id" id="user_id"
                                            ...
                                            <option value="{{ $santri->id }}" data-name="{{ $santri->nama_lengkap }}"
                                                data-nik="{{ $santri->nik }}" {{ old('user_id')==$santri->id ?
                                                'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="nama_santri" id="nama_santri"
                                            ...
                                            value="{{ old('nama_santri') }}" required readonly>
                                        @error('nama_santri')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="point"
                                            ...
                                            value="{{ old('point', 1) }}" min="1" max="10" required>
                                        @error('point')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <small class="form-hint">Format: JPG, JPEG, PNG. Maksimal 5MB</small>
                                    </div>
                                    <!-- Preview Foto -->
                                ...
                                <div class="col-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea name="keterangan" rows="5"
                                            ...
                                            placeholder="Jelaskan detail pelanggaran yang dilakukan..." required>{{ old('keterangan') }}</textarea>
                                        @error('keterangan')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <!-- Info Total Pelanggaran Santri -->
                                <div class="col-12">
                                    <div id="info-pelanggaran" class="alert alert-info" style="display: none;">
                                        <h4 class="alert-title">Informasi Pelanggaran Santri</h4>
                                        <div class="text-muted">
                                            Total Pelanggaran: <strong id="total-pelanggaran">0</strong>x<br>
...
<script>
    ...
    document.getElementById('user_id').addEventListener('change', function() {
        ...
        // Load total pelanggaran santri
        if (this.value) {
            loadTotalPelanggaran(this.value);
        } else {
    ...
    // Load total pelanggaran santri
    function loadTotalPelanggaran(userId) {
        fetch(`/pelanggaran-santri/api/total/${userId}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('total-pelanggaran').textContent = data.total;
                
                let badgeClass = '';
                let badgeText = data.status.status;
                
                if (data.status.status === 'Berat') {
                    badgeClass = 'badge bg-red-500 text-white';
                } else if (data.status.status === 'Sedang') {
                    badgeClass = 'badge bg-yellow-500 text-white';
                } else {
                    badgeClass = 'badge bg-green-500 text-white';
                }
                
                document.getElementById('status-badge').className = badgeClass;
                document.getElementById('status-badge').textContent = badgeText;
                document.getElementById('info-pelanggaran').style.display = 'block';
            })
            .catch(error => {
