...
<div class="row">
        ...
        <div class="card">
            <div class="card-body text-center">
                ...
                <div class="d-flex justify-content-center gap-2 mb-3">
                    <a href="{{ route('masar.jamaah.edit', Crypt::encrypt($jamaah->id)) }}" class="btn btn-sm btn-warning">
                        <i class="ti ti-edit"></i> Edit
                    </a>
                    <a href="{{ route('masar.jamaah.downloadIdCard', Crypt::encrypt($jamaah->id)) }}" 
                        class="btn btn-sm btn-primary" target="_blank">
                        <i class="ti ti-id"></i> Download ID Card
                    </a>
                </div>

                <a href="{{ route('masar.jamaah.index') }}" class="btn btn-sm btn-secondary w-100">
    ...
    <div class="col-lg-8 col-md-6">
        ...
        <div class="card mb-3">
            <div class="card-header">
                <h6 class="mb-0"><i class="ti ti-calendar-week me-2"></i>Rekapitulasi Kehadiran Mingguan Tahun {{ date('Y') }}</h6>
            </div>
            <div class="card-body">
                @php
                    // Get kehadiran mingguan dari database
                    $kehadiranMingguan = $jamaah->kehadiranMingguan()
                        ->where('tahun', date('Y'))
                    ...
                    $totalWeeks = now()->endOfYear()->isoWeek;
                @endphp
                
                @if($kehadiranMingguan->count() > 0)
                <div class="table-responsive">
                    <table class="table table-sm table-bordered">
                        <thead class="table-light">
                            <tr>
                                <th class="text-center" style="width: 15%;">Minggu Ke</th>
                                <th class="text-center" style="width: 35%;">Tanggal Jumat</th>
                                <th class="text-center" style="width: 20%;">Kehadiran</th>
                                <th class="text-center" style="width: 30%;">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($kehadiranMingguan as $kehadiran)
                            <tr class="@if($kehadiran->minggu_ke == $currentWeek && $kehadiran->tahun == $currentYear) table-info @endif">
                                <td class="text-center fw-bold">
                                    Minggu {{ $kehadiran->minggu_ke }}
                                    @if($kehadiran->minggu_ke == $currentWeek && $kehadiran->tahun == $currentYear)
                                        <span class="badge bg-info ms-1">SEKARANG</span>
                                    @endif
                                </td>
                                <td>
                                    @if($kehadiran->tanggal_kehadiran)
                                        {{ $kehadiran->tanggal_kehadiran->format('d M Y (D)') }}
                                    @else
                                        <span class="text-muted">-</span>
                                    @endif
                                </td>
                                <td class="text-center">
                                    @if($kehadiran->jumlah_kehadiran > 0)
                                        <span class="badge bg-success p-2">
                                            <i class="ti ti-check me-1"></i>{{ $kehadiran->jumlah_kehadiran }} x Hadir
                                        </span>
                                    @else
                                        <span class="badge bg-danger p-2">
                                            <i class="ti ti-x me-1"></i>Tidak Hadir
                                        </span>
                                    @endif
                                </td>
                                <td>
                                    <small class="text-muted">
                                        Updated: {{ $kehadiran->last_updated ? $kehadiran->last_updated->format('d M Y H:i') : 'N/A' }}
                                    </small>
                                </td>
                            </tr>
                            @endforeach
                ...
                <div class="row mt-3">
                    <div class="col-md-6">
                        <div class="alert alert-info mb-0">
                            <strong>Total Kehadiran Tahun Ini:</strong>
                            <h4 class="mb-0 mt-2">
                                <span class="text-primary">{{ $kehadiranMingguan->sum('jumlah_kehadiran') }}</span> 
                                dari <span class="text-secondary">{{ $totalWeeks }}</span> minggu
                            </h4>
                        </div>
                    </div>
                    <div class="col-md-6">
                        @php
                            $totalKehadiran = $kehadiranMingguan->sum('jumlah_kehadiran');
                            $percentage = ($totalWeeks > 0) ? round(($totalKehadiran / $totalWeeks) * 100, 1) : 0;
                        @endphp
                        <div class="progress mt-2" style="height: 30px;">
                            <div class="progress-bar bg-{{ $percentage >= 75 ? 'success' : ($percentage >= 50 ? 'warning' : 'danger') }}" 
                                 ...
                                 aria-valuemin="0" 
                                 aria-valuemax="100">
                                <span class="fw-bold text-white">{{ $percentage }}%</span>
                            </div>
                        </div>
                ...
                <div class="text-center text-muted py-4">
                    ...
                    <p class="mt-2">Belum ada data kehadiran mingguan untuk tahun ini</p>
                    <small class="text-muted">Kehadiran akan dicatat setiap hari Jumat saat proses "Get Data Kehadiran" dilakukan</small>
                </div>
                @endif
            </div>
        </div>
