    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto">
                <div class="btn-list">
                    <a href="{{ route('masar.kehadiran.dashboard') }}" class="btn btn-info">
                        <i class="ti ti-chart-line me-1"></i> Dashboard Statistik
                    </a>
                    <a href="{{ route('masar.kehadiran.create') }}" class="btn btn-primary">
...
$(document).ready(function() {
    ...
    var table = $('#tableRekapKehadiran').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('masar.kehadiran.index') }}",
        columns: [
    ...
    $('#tableRekapKehadiran').on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('masar.kehadiran.destroy', ':id') }}".replace(':id', id),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
