@extends('layouts.app')
@section('titlepage', 'Edit Jadwal Khidmat')

@section('content')
@section('navigasi')
    <span><a href="{{ route('khidmat.index') }}">Saung Santri / Khidmat</a> / Edit Jadwal</span>
@endsection
        ...
        <div class="card">
            <div class="card-header bg-gradient-warning">
                <h5 class="text-white mb-0">
                    <i class="ti ti-edit me-2"></i>Edit Jadwal Khidmat
                </h5>
            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('khidmat.update', $jadwal->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_kelompok" id="nama_kelompok" 
                               ...
                               value="{{ old('nama_kelompok', $jadwal->nama_kelompok) }}" required>
                        @error('nama_kelompok')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="tanggal_jadwal" id="tanggal_jadwal" 
                               ...
                               value="{{ old('tanggal_jadwal', $jadwal->tanggal_jadwal->format('Y-m-d')) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="saldo_masuk" id="saldo_masuk" 
                                   ...
                                   value="{{ old('saldo_masuk', $jadwal->saldo_masuk) }}" min="0" step="0.01">
                            @error('saldo_masuk')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="petugas[]" id="petugas" class="form-select @error('petugas') is-invalid @enderror" 
                            ...
                            @foreach($santri as $s)
                                <option value="{{ $s->id }}" 
                                    {{ in_array($s->id, old('petugas', $jadwal->petugas->pluck('santri_id')->toArray())) ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="3" 
                                  ...
                                  placeholder="Catatan tambahan (opsional)">{{ old('keterangan', $jadwal->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
