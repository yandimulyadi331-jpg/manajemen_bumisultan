<div class="row">
    <div class="col">
        <table class="table">
            <tr>
                <th>NIK</th>
                <td>{{ $yayasan_masar->nik }}</td>
            </tr>
            <tr>
                <th>Nama</th>
                <td>{{ $yayasan_masar->nama }}</td>
            </tr>
            <tr>
                <th>Departemen</th>
                <td>{{ textUpperCase($yayasan_masar->nama_dept) }}</td>
            </tr>
            <tr>
                <th>Cabang</th>
                <td>{{ textUpperCase($yayasan_masar->nama_cabang) }}</td>
            </tr>
        ...
        <div class="nav-align-top  mb-6">
            ...
            <div class="tab-content">
                <div class="tab-pane fade active show" id="navs-top-home" role="tabpanel">
                    <form action="{{ route('yayasan_masar.storejamkerjabyday', Crypt::encrypt($yayasan_masar->nik)) }}" id="formSetJamkerja" method="POST">
                        @csrf
...
<script>
    $(document).ready(function() {
        ...
        function addJamKerja(tanggal, kodeJamKerja, namaJamKerja, jamKerja) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('yayasan_masar.storejamkerjabydate') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    nik: "{{ $yayasan_masar->nik }}",
                    tanggal: tanggal,
        ...
        function replaceJamKerja(tanggal, kodeJamKerja, namaJamKerja, jamKerja) {
            ...
            if (existingJamKerja) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('yayasan_masar.deletejamkerjabydate') }}",
                    data: {
                        _token: "{{ csrf_token() }}",
                        nik: "{{ $yayasan_masar->nik }}",
                        tanggal: tanggal,
        ...
        function loadJamKerjaData() {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('yayasan_masar.getjamkerjabydate') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    nik: "{{ $yayasan_masar->nik }}",
                    bulan: bulan,
                    ...
                    respond.map((d) => {
                        ...
                        $(document).find("#getjamkerjabydate").append(`
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="deletejamkerjabydate" tanggal="${d.tanggal}" kode_yayasan="${d.nik}">
