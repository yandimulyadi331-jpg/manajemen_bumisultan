...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (!Schema::hasTable('presensi_yayasan')) {
            Schema::create('presensi_yayasan', function (Blueprint $table) {
                $table->id();
                $table->char('kode_yayasan', 8);
                $table->date('tanggal');
                ...
                $table->char('status', 1);
                $table->foreign('kode_yayasan')->references('kode_yayasan')->on('yayasan_masar')->restrictOnDelete()->cascadeOnUpdate();
    ...
    {
        Schema::dropIfExists('presensi_yayasan');
    }
};
